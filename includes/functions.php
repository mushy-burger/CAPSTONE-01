...
function ensureMultiServiceBookingSchema(): void {
    ...
    try {
        $db->exec("ALTER TABLE service_types ADD COLUMN required_category VARCHAR(120) DEFAULT NULL AFTER applies_to");
    } catch (Throwable $e) {
    }

    try {
        $db->exec("ALTER TABLE service_types ADD COLUMN required_category_id INT UNSIGNED DEFAULT NULL AFTER required_category");
    } catch (Throwable $e) {
    }
